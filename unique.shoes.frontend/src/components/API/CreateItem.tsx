...
const handleItemAdd = async (item_name: string, item_price: number, item_desc: string, item_sizes: string[], accessToken: string, retry: boolean = true) : Promise<any> => {
            ...
            if (error.response) {
                if (error.response.status === 401 && retry) {
                    ...
                    if (await TokenNeedUpdate()) {
                        ...
                        return handleItemAdd(item_name, item_price, item_desc, item_sizes, accessTokens, false);
                    }
                }
                else {
